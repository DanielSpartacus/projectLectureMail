...
public class MailCaptureUtilitaire {
    private static final Integer INTEGER_ZERO = 0;

    /**
     * Convert a message from the protocol EWS to a {@link Mail} object
     *
     * @param email The {@link microsoft.exchange.webservices.data.core.service.item.EmailMessage
     * }
     * @return The converted {@link model.Mail} object
     */
    public static Mail convertEwsEmailMessageToMail(EmailMessage email) throws MailException {
        ...
        if (email != null) {
            ...
            result = new Mail<>();
            result.setMessage((EmailMessage)email);
            ...
            try {
                ...
                if (email.getHasAttachments()) {
                    ...
                    for (int i = INTEGER_ZERO; i < attachmentCollection.getCount(); i++) {
                        ...
                        attachement.setAttachment(fileAttachment);
                        result.getAttachments().add(attachement);
                    }
                }
            }
            catch (Exception e) {
                throw new MailException("An error has occured when converting the ews mail message to the Mail object", e);
            }
        }
                ...
                if (message.getContent() instanceof Multipart) {
                    ...
                    for (Integer i = INTEGER_ZERO; i < multipart.getCount(); i++) {
                        ...
                        if (Part.ATTACHMENT.equalsIgnoreCase(bodyPart.getDisposition()) &&
                            ...
                            Attachement attachement = new Attachement();
                            attachement.setAttachment(bodyPart);
                            result.getAttachments().add(new Attachement());
                        }
                    }
                }
            }
            catch (MessagingException | IOException e) {
                new MailException("An error has occured when parsing the mail contents", e);
            }
        }
    ...
    private  static String getBodyContent(Part part) throws MessagingException, IOException {
        ...
        if (part.isMimeType(TypeMailConstant.TEXT_ALL) &&
        ...
        else if (part.isMimeType(TypeMailConstant.MULTIPART_ALTERNATIVE)) {
            ...
            for (Integer i = 0; i < multiPart.getCount(); i++) {
                ...
                if (bodyPart.isMimeType(TypeMailConstant.TEXT_PLAIN)) {
                ...
                else if (bodyPart.isMimeType(TypeMailConstant.TEXT_HTML)) {
