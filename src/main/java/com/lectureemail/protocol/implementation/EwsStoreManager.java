package com.lectureemail.protocol.implementation;
...
import exceptions.EwsStoreManagerException;
...
public class EwsStoreManager implements IlectureEmail {
    ...
    @Override
    public List<Mail> listeMail() throws EwsStoreManagerException {
                ...
                try {
                    ...
                    if (CollectionUtils.isNotEmpty(findResults.getItems())) {
                        ...
                        findResults.getItems()
                                .forEach(email -> {
                                    try {
                                        messageList.add(MailCaptureUtilitaire.convertEwsEmailMessageToMail((EmailMessage) email));
                                    }
    ...
    private SearchFilter getFilters() throws ConfigurationException {
        ...
        String senderFilter = Configuration.getInstance().getPropertyValueAsString(FiltreMailConstant.MAIL_FILTER_FROM);
        String subjectFilter = Configuration.getInstance().getPropertyValueAsString(FiltreMailConstant.MAIL_FILTER_SUBJECT);
        String hasAttachmentsFilter = Configuration.getInstance().getPropertyValueAsString(FiltreMailConstant.MAIL_FILTER_HAS_ATTACHMENT);
        Boolean hasAttachments = (StringUtils.isNotBlank(hasAttachmentsFilter) ? Configuration.getInstance().getPropertyValueAsBoolean(FiltreMailConstant.MAIL_FILTER_HAS_ATTACHMENT) : false);
